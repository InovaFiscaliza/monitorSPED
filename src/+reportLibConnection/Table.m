        ...
        function jsonFileContent = scarabJsonFile(projectData, context, ecdObj, correlationKey, executionMode, issueDetails)
            ...
            for ii = 1:numel(ecdObj)
                ...
                taxEstimateSummary(end+1) = struct( ...
                    ...
                    'robTelecom',             round(ecdObj(ii).Table.x_APURACAO_GERAL{'ROB TELECOM',                    'TOTAL'}, 2), ...
                    'icmsEstimado',           round(ecdObj(ii).Table.x_APURACAO_GERAL{'ICMS ESTIMADO',                  'TOTAL'}, 2), ...
                    'icmsContabil',           round(ecdObj(ii).Table.x_APURACAO_GERAL{'ICMS CONTÁBIL',                  'TOTAL'}, 2), ...
                    'baseCalculoPisCofins',   round(ecdObj(ii).Table.x_APURACAO_GERAL{'BÁSE DE CÁLCULO (PIS/COFINS)',   'TOTAL'}, 2), ...
                    'pisEstimado',            round(ecdObj(ii).Table.x_APURACAO_GERAL{'PIS ESTIMADO',                   'TOTAL'}, 2), ...
                    'pisContabil',            round(ecdObj(ii).Table.x_APURACAO_GERAL{'PIS CONTÁBIL',                   'TOTAL'}, 2), ...
                    'cofinsEstimado',         round(ecdObj(ii).Table.x_APURACAO_GERAL{'COFINS ESTIMADO',                'TOTAL'}, 2), ...
                    'cofinsContabil',         round(ecdObj(ii).Table.x_APURACAO_GERAL{'COFINS CONTÁBIL',                'TOTAL'}, 2), ...
                    'baseCalculoFustFunttel', round(ecdObj(ii).Table.x_APURACAO_GERAL{'BÁSE DE CÁLCULO (FUST/FUNTTEL)', 'TOTAL'}, 2), ...
                    'valorApuradoFust',       round(ecdObj(ii).Table.x_APURACAO_GERAL{'VALOR APURADO FUST',             'TOTAL'}, 2), ...
                    'valorApuradoFunttel',    round(ecdObj(ii).Table.x_APURACAO_GERAL{'VALOR APURADO FUNTTEL',          'TOTAL'}, 2) ...
                );
